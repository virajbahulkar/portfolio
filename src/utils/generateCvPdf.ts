/* Utility to generate a compact CV PDF, adding pages rather than removing content */
...
export function generateCvPdf(jsPDF: typeof JsPDFType, data: CvData) {
  ...
  while (attempt.overflow && config.fonts.body.size > 10) {
    ...
    config.fonts.body.lineHeight = Math.max(
      11.5,
      config.fonts.body.lineHeight - 0.25
    );
...
function buildBlocks(
  ...
  const paragraph = (text: string, style: FontStyle): Block => ({
    type: 'paragraph',
    lines: wrap(text, style),
    style,
  });
  const heading = (t: string): Block => ({
    type: 'heading',
    lines: [t],
    style: config.fonts.heading,
  });
  const bullet = (t: string): Block => ({
    type: 'bullet',
    lines: wrap(`• ${t}`, config.fonts.bullet),
    style: config.fonts.bullet,
  });

  // Name & Contact
  blocks.push({
    type: 'name',
    lines: wrap(`${data.profile.name} - Resume`, config.fonts.name),
    style: config.fonts.name,
  });
  ...
  data.experience.forEach((exp) => {
    ...
    blocks.push({
      type: 'role',
      lines: wrap(`${exp.title}${duration}`, config.fonts.sub),
      style: config.fonts.sub,
    });
    (exp.projects || []).forEach((p) => {
      blocks.push({
        type: 'project',
        lines: wrap(p.name, config.fonts.body),
        style: config.fonts.body,
      });
    ...
    if (exp.technologies)
      blocks.push(
        paragraph(`Technologies: ${exp.technologies}`, config.fonts.body)
      );
  });
...
function buildDuration(start?: string, end?: string) {
  ...
  const startText = s.toLocaleString('default', {
    month: 'short',
    year: 'numeric',
  });
  const endText = (end ? new Date(end) : new Date()).toLocaleString('default', {
    month: 'short',
    year: 'numeric',
  });
  const parts: string[] = [];
...
function layout(
  ...
  const bottom = page.height - config.margins.bottom;
  const usable = bottom - config.margins.top;
  const nextColumn = () => {
  ...
  blocks.forEach((b, index) => {
    const lines = b.lines || [''];
    const h = lines.length * b.style.lineHeight + 4;
    const next = blocks[index + 1];
    const keepWithNext = ['heading', 'role', 'project', 'edu'].includes(b.type);
    const nextHeight =
      keepWithNext && next
        ? (next.lines?.length || 1) * next.style.lineHeight + 4
        : 0;

    const previous = current[current.length - 1];
    const followsHeading =
      previous && ['heading', 'role', 'project', 'edu'].includes(previous.type);
    if (
      current.length &&
      !(h > usable && followsHeading) &&
      y + Math.min(h + nextHeight, usable) > bottom
    ) {
      nextColumn();
    }
    // Split oversized paragraphs across columns without dropping any lines.
    let offset = 0;
    while (offset < lines.length) {
      const capacity = Math.floor((bottom - y - 4) / b.style.lineHeight);
      if (capacity < 1) {
        nextColumn();
      } else {
        const part = lines.slice(offset, offset + capacity);
        const height = part.length * b.style.lineHeight + 4;
        current.push({ ...b, lines: part, height });
        y += height;
        offset += part.length;
        if (offset < lines.length) nextColumn();
      }
    }
  });
  if (current.length) columns.push(current);
  return { overflow: columns.length > 4, columns };
}
...
function render(
  ...
  columns.forEach((blocks, i) => {
    ...
    const xBase =
      config.margins.left +
      (i % 2 === 1 ? colWidth + config.margins.gutter : 0);
    let y = config.margins.top;
    blocks.forEach((b) => {
      doc.setFont(...b.style.font);
      doc.setFontSize(b.style.size);
      if (b.lines) {
        b.lines.forEach((line) => {
          y += b.style.lineHeight;
