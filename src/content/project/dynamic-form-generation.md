---
title: 'Dynamo formik'
description: 'Configuration-driven form platform for generating reusable forms, validation rules, and backend integrations without rebuilding each flow manually.'
startDate: 2022-05-25T00:00:00Z
openSource: false
category: NodeJs, React, MongoDB, PL/Sql, Azure DevOps
runningOrder: 2
---

<div class=" shadow-xl rounded-2xl p-6 mb-8 ">
  <h2 class="text-2xl font-semibold ">Problem</h2>
  <p class="mt-2 pt-2">
    Teams needed a faster way to create and maintain complex form workflows
    while keeping validation and data rules consistent.
  </p>

  <hr class=" mb-8 mt-8 border border-base-300">

  <h2 class="text-2xl font-semibold  ">What I built</h2>
  <ul class="list-disc list-inside mt-4 pt-2">
    <li>Configuration-driven form generation using reusable schemas and validation rules.</li>
    <li>React-based form rendering connected to NodeJS services.</li>
    <li>Integrations with MongoDB and PL/SQL-backed data handling.</li>
  </ul>

  <hr class=" mb-8 mt-8 border border-base-300">

  <h2 class="text-2xl font-semibold ">Delivery environment</h2>
  <ul class="list-disc list-inside mt-4 pt-2">
    <li><strong>Frontend:</strong> ReactJS and Formik</li>
    <li><strong>Backend:</strong> NodeJS</li>
    <li><strong>Data:</strong> MongoDB and PL/SQL workflows</li>
    <li><strong>CI/CD:</strong> Azure DevOps</li>
  </ul>
</div>
